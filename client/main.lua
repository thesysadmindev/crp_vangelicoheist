...
RegisterNetEvent('crp_vangelicoheist:currentlyrobbing')
AddEventHandler('crp_vangelicoheist:currentlyrobbing', function(robb)
	holdingup = true
	store = robb
end)

RegisterNetEvent('crp_vangelicoheist:killblip')
AddEventHandler('crp_vangelicoheist:killblip', function()
    RemoveBlip(blipRobbery)
end)

RegisterNetEvent('crp_vangelicoheist:setblip')
AddEventHandler('crp_vangelicoheist:setblip', function(position)
	...
	while true do
		...
		if holdingup then
			...
			for i,v in pairs(vetrine) do 
				...
				if(GetDistanceBetweenCoords(pos, v.x, v.y, v.z, true) < 0.75) and not v.isOpen then 
					...
					if IsControlJustPressed(0, 38) then
					    ...
					    TriggerServerEvent('crp_vangelicoheist:gioielli')
						...
						if vetrineRotte == Config.MaxWindows then 
							...
							TriggerServerEvent('crp_vangelicoheist:endrob', store)
			...
			if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), -622.566, -230.183, 38.057, true) > 11.5 ) then
				TriggerServerEvent('crp_vangelicoheist:toofar', store)
				holdingup = false
				for i,v in pairs(vetrine) do 
					v.isOpen = false
					vetrineRotte = 0
